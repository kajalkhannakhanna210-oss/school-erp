...
const styles = StyleSheet.create({
  ...
  label: { color: "#555555" },
  table: { marginTop: 16, borderTopWidth: 1, borderTopColor: "#dddddd" },
  tableRow: {
    flexDirection: "row",
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.table}>
          <View style={styles.tableRow}>
            ...
            {/* "Rs." rather than the rupee glyph — the default Helvetica
                base font doesn't include it, and a broken glyph on a
                financial document is worse than the abbreviation. A custom
                font that covers it would fix this properly. */}
            <Text>Rs. {payment.amount.toFixed(2)}</Text>
          </View>
          <View style={styles.totalRow}>
            <Text style={styles.totalLabel}>Total Paid</Text>
            <Text style={styles.totalLabel}>Rs. {payment.amount.toFixed(2)}</Text>
          </View>
        </View>

        <View style={styles.footer}>
          <Text style={styles.label}>Payment Mode: Online (Razorpay)</Text>
          {payment.razorpay_payment_id && (
            <Text style={styles.label}>Transaction ID: {payment.razorpay_payment_id}</Text>
          )}
        </View>
      </Page>
    </Document>
...
export async function renderReceiptPdf(payment: ReceiptPayment) {
  return renderToBuffer(<ReceiptDocument payment={payment} />);
}
