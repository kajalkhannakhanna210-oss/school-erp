...
function ReportDocument({ result, generatedAt }: { result: ReportResult; generatedAt: string }) {
  return (
    <Document>
      <Page size="A4" orientation="landscape" style={styles.page}>
        ...
        <View style={styles.headerRow}>
          {result.columns.map((c) => (
            <Text key={c.key} style={[styles.headerCell, c.align === "right" ? styles.right : undefined]}>
              {c.label}
            </Text>
          ))}
        </View>
        {result.rows.map((row, i) => (
          <View key={i} style={styles.row}>
            {result.columns.map((c) => (
              <Text key={c.key} style={[styles.cell, c.align === "right" ? styles.right : undefined]}>
                {String(row[c.key] ?? "")}
              </Text>
            ))}
          </View>
        ))}
        {result.rows.length === 0 && <Text style={styles.empty}>No data for the selected filters.</Text>}
      </Page>
    </Document>
  );
}
...
export async function renderReportPdf(result: ReportResult) {
  ...
  return renderToBuffer(<ReportDocument result={result} generatedAt={generatedAt} />);
}
