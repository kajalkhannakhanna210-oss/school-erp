...
export default async function RoleAccessPage() {
  ...
  const [{ data: organizations }, { data: schools }, { data: staff }, { data: schoolScopes }] = await Promise.all([
    ...
    admin.from("staff").select("id, employee_id, organization_id, profiles(full_name)").eq("is_active", true).order("employee_id"),
  ...
  return (
    <div>
      ...
      <RoleAccessForm
        ...
        staff={(staff ?? []).map((member: any) => ({ id: member.id, employee_id: member.employee_id, organization_id: member.organization_id, full_name: member.profiles?.full_name ?? member.employee_id }))}
