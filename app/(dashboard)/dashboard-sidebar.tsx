...
export function DashboardSidebar({ items }: { items: NavItem[] }) {
  ...
  return <aside className={`hidden border-r border-ink-900 p-4 transition-all duration-200 lg:block ${collapsed ? "w-20" : "w-[260px]"}`} style={{ backgroundColor: "#222F57" }}>
    <div className={`flex items-center ${collapsed ? "justify-center" : "justify-between"}`}>
      {!collapsed && <div className="flex items-center gap-3 px-2"><div className="grid h-9 w-9 place-items-center rounded-xl bg-gold text-sm font-bold text-ink-900">R</div><div><div className="font-display text-lg font-bold tracking-tight text-white">Registrar</div><p className="text-xs text-white/60">School Management</p></div></div>}
    ...
    <div className="mt-2"><SignOutButton collapsed={collapsed} /></div>
  </aside>;
}
