...
export function ArchiveControl({ studentId, isActive, open: openProp, onClose, hideTrigger = false }: { studentId: string; isActive: boolean; open?: boolean; onClose?: () => void; hideTrigger?: boolean }) {
  ...
  function handleConfirm() {
    if (isActive && !archiveDate) {
      push("Please select an archive date", "error");
      return;
    }
    if (!remark.trim()) {
      push(isActive ? "Please enter a remark for archiving" : "Please enter a remark for restoring", "error");
      return;
    }
    ...
    startTransition(async () => {
      const { error } = isActive
        ...
        : await restoreStudent(studentId, remark.trim());
        ...
        <div className="p-6">
          ...
          <div className="mt-4 space-y-2">
            <Label htmlFor="remark">
              {isActive ? "Remark / Reason for Archiving *" : "Remark / Reason for Restoring *"}
            </Label>
            <Textarea
              id="remark"
              placeholder={
                isActive
                  ? "Enter the reason for archiving this student..."
                  : "Enter the reason for restoring this student..."
              }
              value={remark}
          ...
          <div className="mt-6 flex justify-end gap-3">
            ...
            <Button
              ...
              disabled={pending || !remark.trim() || (isActive && !archiveDate)}
            >
