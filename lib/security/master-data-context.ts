...
export async function getMasterDataContext() {
  ...
  if (!auth.user) return { userId: null, role: null, organizationId: null, schoolId: null, loginScope: null, schools: [] as MasterSchool[] };
  ...
  return { userId: auth.user.id, role, organizationId, schoolId: loginContext?.schoolId ?? null, loginScope: loginContext?.loginScope ?? null as LoginScope | null, schools: visibleSchools as MasterSchool[] };
}
