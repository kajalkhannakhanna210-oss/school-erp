...
export async function getPageMetadata(path: string, fallback: Metadata): Promise<Metadata> {
  try {
    const { data } = await createPublicClient().from("site_seo_metadata").select("title, description, keywords, canonical_path, og_title, og_description, og_image, indexable").eq("path", path).maybeSingle();
